      ...
      @if (depreciacoes().length > 0) {
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 animate-fade-in-up">
          ...
          <div class="bear-card p-4 flex items-center gap-4">
            ...
            <div><p class="text-xs font-medium" style="color:var(--text-secondary)">Deprec. Período</p><p class="text-xl font-bold ink-error">{{ totalPeriodo() | currency:'BRL' }}</p></div>
          </div>
          <div class="bear-card p-4 flex items-center gap-4">
            <div class="stat-icon stat-icon--teal"><span class="material-symbols-rounded">account_balance</span></div>
            <div><p class="text-xs font-medium" style="color:var(--text-secondary)">Valor Residual Total</p><p class="text-xl font-bold ink-teal">{{ totalResidual() | currency:'BRL' }}</p></div>
          </div>
        </div>
      }
      ...
      @if (!loading() && depreciacoes().length > 0) {
        <div class="bear-card overflow-hidden animate-fade-in-up">
          ...
          <table mat-table [dataSource]="depreciacoes()" class="w-full">
            ...
            <ng-container matColumnDef="valorDepreciacao"><th mat-header-cell *matHeaderCellDef>Valor Deprec.</th><td mat-cell *matCellDef="let d" class="font-semibold ink-error">{{ d.valorDepreciacao | currency:'BRL' }}</td></ng-container>
            ...
            <ng-container matColumnDef="valorResidualAtual"><th mat-header-cell *matHeaderCellDef>Valor Residual</th><td mat-cell *matCellDef="let d" class="ink-teal">{{ d.valorResidualAtual | currency:'BRL' }}</td></ng-container>

...
| 🟡 **P1.3** Folha patronal — **engine FEITA**: `folha-patronal.ts` (`calcularInssPatronal` 20% SEM teto; `ratEfetivo` RAT×FAP; `calcularEncargosPatronais` INSS+RAT+terceiros; flag `recolhePatronal` p/ Simples). `folha-patronal.spec.ts` (node 5/5 + tsc). Schema `empresas` += `rat/fap/codFpas/aliqTerceiros` (gate rodado). **Form da empresa capturando RAT/FAP/FPAS/terceiros FEITO** (tsc verde). **Falta (UI):** exibir/agregar a patronal no holerite por competência + feed S-1200/DCTFWeb. | `folha-patronal.ts`, `empresas.component`, `holerites` (falta) | Médio (2–3 d) |
...
| ✅ **P1.6** Código referencial RFB (I051) — gerador + **wiring FEITO**: `EcdConta.codCtaRef` + registro I051 em `ecd-sped.generator.ts`; form do plano de contas captura `codCtaRef` (doc→view→payload + input); `montarDadosEcd` repassa a `codCtaRef`. `contabilidade-ecd.spec.ts` (node) + tsc verde. Schema `plano_contas.codCtaRef` (gate rodado). | `ecd-sped.generator.ts`, `contabilidade.service.ts`, `plano-contas.component`, `teste-bear` | Médio (2–3 d) |
